...
const Dashboard = () => {
  ...
  const handleDeleteJob = async () => {
    ...
    // Delete related data first, then the job
    await Promise.all([
      supabase.from("screening_results").delete().eq("job_id", deleteTarget.id).eq("user_id", user.id),
      supabase.from("candidates").delete().eq("job_id", deleteTarget.id).eq("user_id", user.id),
    ]);
    const { error } = await supabase.from("jobs").delete().eq("id", deleteTarget.id).eq("user_id", user.id);
    if (error) {
      toast.error("Failed to delete job: " + error.message);
    } else {
      toast.success(`"${deleteTarget.title}" has been deleted.`);
      setRecentJobs((prev) => prev.filter((j) => j.id !== deleteTarget.id));
        ...
        <div className="flex-1 min-w-0 space-y-6">
          ...
          <div className="flex items-start justify-between">
            ...
            <div className="flex gap-2 shrink-0">
              {stats.jobs === 0 && !localStorage.getItem("demo_loaded") && (
                <Button
                  variant="outline"
                  size="sm"
                  onClick={loadDemo}
